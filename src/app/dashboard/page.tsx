...
async function getGroupsSafe() {
  try {
    ...
    return {
      groups: [],
      error:
        "Não foi possível carregar seus grupos. Tente recarregar a página.",
    };
  }
}
...
export default async function DashboardPage() {
  ...
  return (
    <div className="mx-auto w-full flex-1 px-6 py-10 sm:px-8 lg:px-12 xl:px-16">
      {/* ════════════════════════════════════════════════ */}
      {/* WELCOME */}
      {/* ════════════════════════════════════════════════ */}
      <div className="mb-12">
      ...
      <div className="mb-12">
        <div className="mb-5 flex items-center gap-3">
          <span className="font-pixel text-[10px] tracking-wider text-retro-primary/60 uppercase">
            $ VISÃO GERAL
          </span>
          <div className="h-px flex-1 bg-gradient-to-r from-retro-border/50 to-transparent" />
        </div>
      ...
      <section>
        <div className="mb-5 flex items-center gap-3">
          ...
          {groups.length > 0 && !groupsError && (
            <span className="inline-flex items-center justify-center pixel-border-sm bg-retro-primary/15 px-2 py-0.5 font-pixel text-[8px] text-retro-primary">
              {groups.length}
            </span>
          )}
          <div className="h-px flex-1 bg-gradient-to-r from-retro-border/50 to-transparent" />
        ...
        {/* Error banner */}
        {groupsError && (
          <div className="mb-6 border border-retro-red/20 bg-retro-red/5 p-5">
            <div className="flex items-center gap-3 mb-3">
              <svg
                className="h-5 w-5 shrink-0 text-retro-red"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={1.5}
                stroke="currentColor"
            ...
            <p className="mb-4 font-pixel text-[8px] text-retro-text-dim">
              {groupsError}
            </p>
            <form action="/dashboard" method="GET">
              <button
                type="submit"
                className="pixel-btn border-retro-border/30 px-4 py-2 text-[8px] text-retro-text-dim hover:text-retro-text"
              >
                <svg
          ...
          <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
            {groups.map((group, index) => (
              <Link
                key={group.id}
                href={`/group/${group.id}`}
                className="glass-card group relative overflow-hidden border border-retro-border/20 p-5 transition-all hover:-translate-y-0.5 hover:border-retro-primary/20 hover:cyber-glow-sm"
              ...
              >
                ...
                <div className="relative z-10">
                  <div className="mb-4 flex items-start justify-between">
                    ...
                    <span
                      className={`shrink-0 ml-2 px-2 py-0.5 font-pixel text-[7px] pixel-border-sm ${
                        group.role === "OWNER"
                          ? "bg-retro-primary/15 text-retro-primary border-retro-primary/20"
                  ...
                  <div className="flex items-center gap-5">
                    <span className="flex items-center gap-1.5">
                      <svg
                        className="h-3.5 w-3.5 text-retro-text-dim/60"
                        fill="none"
                        viewBox="0 0 24 24"
                        strokeWidth={1.5}
                        stroke="currentColor"
                    ...
                    <span className="flex items-center gap-1.5">
                      <svg
                        className="h-3.5 w-3.5 text-retro-text-dim"
                        fill="none"
                        viewBox="0 0 24 24"
                        strokeWidth={1.5}
                        stroke="currentColor"
                      ...
                      <span className="font-pixel text-[8px] text-retro-text-dim">
                        {group._count.games}{" "}
                        {group._count.games === 1 ? "jogo" : "jogos"}
                      </span>
                    </span>
                  </div>
                </div>
